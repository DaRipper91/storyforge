...
## 2026-07-15 - Prevent Authorization Bypass in Lobby Routes
**Vulnerability:** Endpoints handling lobby mechanics (like joining, leaving, and updating names) accepted an optionally authenticated `controller_id`. When attempting to resolve Google users via a `storyforge_session` cookie, if the token was absent or invalid, the logic simply fell back to blindly accepting the user-provided `controller_id` string from the request body. This allowed an unauthenticated attacker to inject a `controller_id` starting with `"google::"` (e.g. `"google::12345"`) and successfully spoof the identity of an authenticated user.
**Learning:** In endpoints supporting "mixed mode" authentication (where both authenticated users and anonymous guests are allowed), fallback mechanisms can become bypasses. If the API logic allows a client-provided identity string to mimic the format of a verified server-derived identity, spoofing is trivial.
**Prevention:** Always namespace or strictly isolate unverified guest identities from verified authenticated identities. If a client attempts to claim a privileged namespace (like `"google::"`) via user input, the server must explicitly validate that claim against a secure token and outright reject the request (e.g., HTTP 403) if the token is missing or invalid, rather than passively falling back to the unverified input.
