...
## 2026-06-11 - [Prevent IDOR / Identity Spoofing in Mixed-Auth Fastapi Endpoints]
**Vulnerability:** Fastapi endpoints that optionally accept authentication but fallback to a client-provided `controller_id` (e.g. for guest play) are vulnerable to identity spoofing if the client-provided `controller_id` is permitted to mimic authenticated identity prefixes (like `google::`).
**Learning:** Mixed-auth endpoints must explicitly partition the identifier space. If an endpoint uses `google::sub` for authenticated users, it must actively reject any client-provided fallback ID that starts with `google::` to prevent unauthenticated users from assuming the identity of authenticated users. Additionally, when refactoring logic into helpers, ensure all mutating endpoints (like `save_draft`) actually call the helper; endpoints that omit it entirely bypass all auth.
**Prevention:** In mixed-auth systems, encapsulate identity resolution into a single secure helper function that prioritizes cryptographic session tokens, explicitly blocks spoofing prefixes in fallback inputs, and is consistently applied to all relevant endpoints.
