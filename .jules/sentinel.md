...
## 2024-07-20 - Fix controller_id spoofing in lobby routes
**Vulnerability:** Authorization bypass. The lobby endpoints (`join_lobby`, `leave_lobby`, `update_name`, `save_draft`) allowed unauthenticated users to impersonate authenticated users by manually supplying a `controller_id` prefixed with `google::`. This occurred because the endpoints trusted the client-provided `controller_id` if the JWT decode failed or was omitted.
**Learning:** In mixed-auth endpoints where a `controller_id` is used as a fallback for guest users, the backend must explicitly verify that any `controller_id` claiming an authenticated namespace (like `google::`) is backed by a valid, decoded JWT session.
**Prevention:** Always track `is_authenticated` status when decoding tokens. If a request claims an authenticated prefix but `is_authenticated` is false, reject it with a 403. Ensure this logic is applied universally to all endpoints accepting the mixed-auth payload.
