...
## 2026-07-22 - Fix authorization bypass in lobby API
**Vulnerability:** A malicious client could bypass authentication and spoof another user's authenticated `controller_id` (e.g. `google::<id>`) on the `/api/lobby/*` endpoints by sending a request without a `storyforge_session` token, allowing them to hijack or tamper with another user's lobby slot.
**Learning:** In mixed-auth endpoints where users can be either unauthenticated guests (with client-provided `controller_id`) or authenticated users (with a server-decoded token), explicitly rejecting client-provided IDs that mimic authenticated prefixes (like `google::`) is essential when no valid token is present. Using `pass` in a bare `except Exception:` block left the token string truthy but invalid, which bypassed subsequent fallback logic checks.
**Prevention:** Always implement a spoofing prevention check (e.g. `if not token and controller_id and controller_id.startswith("google::")`) in mixed-auth endpoints. Ensure that invalid tokens are explicitly set to `None` in exception blocks so they do not accidentally satisfy `if not token:` conditional logic.
