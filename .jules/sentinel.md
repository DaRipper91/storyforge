...
## 2026-06-09 - Spoofing Vulnerability in Mixed-Auth Endpoints
**Vulnerability:** Endpoints allowing both unauthenticated guest play (via `controller_id`) and authenticated play (via Google JWT) were vulnerable to identity spoofing. Malicious actors could supply a guest `controller_id` formatted as `google::<id>`, bypassing authentication. Furthermore, if an invalid JWT was provided, the exception block caught the error but failed to clear the `token` variable, allowing the execution to fall through and assume the invalid token was truthy.
**Learning:** Mixed-authentication endpoints require explicit state management for failed authentication attempts. A caught exception does not automatically invalidate the presence of a token variable in Python. Moreover, when fallback guest mechanisms coexist with authenticated identities, the system must rigidly enforce boundaries between the two identity namespaces.
**Prevention:** Always explicitly set `token = None` (or equivalent) in exception handlers for failed JWT decoding. Explicitly reject any client-provided unauthenticated guest identifiers that attempt to use authenticated identity prefixes (e.g., blocking `controller_id.startswith("google::")` if no valid token is present).
