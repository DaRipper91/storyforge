...
## 2024-06-21 - Authorization Bypass in Mixed-Auth API Endpoints
**Vulnerability:** API endpoints handling both guest and authenticated users allowed unauthenticated users to spoof another authenticated user's ID by directly providing a `controller_id` that starts with `"google::"`. The JWT parsing `except Exception: pass` logic left the invalid token truthy, bypassing intended checks.
**Learning:** When APIs support optional or mixed authentication (like session tokens and client-provided guest IDs), you cannot assume that an invalid token will safely fall back. Failing to explicitly clear the invalid token string allows authorization checks further down to falsely pass.
**Prevention:** Always set `token = None` explicitly inside the exception block for failed JWT decoding. Furthermore, explicitly validate that client-provided IDs (like `controller_id`) cannot match the format used for internal authenticated user IDs (e.g., blocking `google::` prefixes).
