...
## 2026-06-11 - Prevent Authorization Spoofing via Controller ID
**Vulnerability:** In mixed-auth endpoints like `join_lobby`, `leave_lobby`, and `update_name` (which accept either an authenticated Google ID or a raw, unauthenticated guest controller ID), a failure in JWT token decoding (e.g., an invalid token) left the original token string intact and skipped payload extraction via an `except Exception: pass` block. This allowed a malicious actor to supply an invalid token alongside a spoofed `controller_id` (e.g., `google::[target_id]`) in the request body. Since the token didn't decode, the `controller_id` wasn't overwritten by the valid payload, but because the code subsequently failed to explicitly block spoofed authenticated prefixes, the request proceeded with the forged identity, resulting in an Insecure Direct Object Reference (IDOR) / Authentication Bypass.
**Learning:** When combining authenticated and unauthenticated pathways in the same endpoint, explicitly clear invalid credentials (`token = None` on decode failure) so subsequent logic does not falsely assume a valid session. Furthermore, never implicitly trust a client-provided identifier that uses reserved, internal formatting (like a `google::` prefix) indicating an authenticated identity.
**Prevention:** Always implement explicit boundary checks for reserved identifier formats. If an endpoint accepts a raw identifier (like `controller_id`) as a fallback for guest users, explicitly reject the request with a 403 Forbidden if the raw identifier uses an authenticated prefix but no valid, verified session token accompanies the request.
