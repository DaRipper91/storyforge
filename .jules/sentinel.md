...
## 2026-06-25 - IDOR/Spoofing vulnerability in mixed-auth endpoints
**Vulnerability:** The `save_draft` endpoint lacked JWT verification entirely, and `join_lobby`, `leave_lobby`, and `update_name` endpoints failed to clear the `token` variable on decode failure. This allowed an attacker to spoof any user's draft or session by manipulating the `controller_id` to start with `"google::"`, bypassing authentication checks.
**Learning:** In mixed-auth endpoints where clients can optionally provide an ID (like `controller_id`), backend logic must strictly derive authenticated identity from the session token and explicitly reject any client-provided ID that attempts to mimic an authenticated identity prefix. Failing to clear a truthy `token` variable after a decode exception can lead to bypasses of subsequent `if not token:` checks.
**Prevention:** When implementing spoofing prevention, always set `token = None` in the `except` block for failed JWT decodes. Enforce a strict check (`if not token and controller_id and controller_id.startswith("google::"): raise HTTPException(403)`) to prevent unauthenticated users from claiming authenticated identities.
