...
## 2024-12-14 - Avoid array serialization for rendering cache keys
**Learning:** Continuously serializing large arrays into strings via methods like `array.join(',')` to generate cache keys in high-frequency rendering functions (like `renderSprite`) causes significant Garbage Collection churn.
**Action:** Construct cache keys using available metadata (e.g., race ID, frame index, direction) by passing them as a cacheId instead of serializing the raw pixel data array.
