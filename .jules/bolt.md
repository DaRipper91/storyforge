...
## 2024-10-25 - Avoid Array Serialization in Cache Keys
**Learning:** Generating Map cache keys for high-frequency rendering functions by serializing large arrays (like 64-element sprite frames) via `array.join(',')` creates severe Garbage Collection churn and micro-stutters.
**Action:** Construct cache keys using available metadata (e.g., race ID, frame index, direction) rather than continuously serializing arrays.
