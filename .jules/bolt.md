...
## 2024-05-20 - Throttling High-Frequency DOM Events
**Learning:** High-frequency DOM events (like `mousemove` or `scroll`) that continuously trigger timer operations (`clearTimeout` and `setTimeout`) cause unnecessary micro-allocations and severe Garbage Collection churn, leading to performance degradation.
**Action:** Optimize these event handlers by implementing a throttle (e.g. tracking `Date.now()`) to reduce execution frequency without breaking the user experience (e.g. idle timer resets).
