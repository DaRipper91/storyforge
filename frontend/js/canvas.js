...
export class GridCanvas {
  ...
  _renderCursor() {
    ...
    if (!this._outerGlow) {
      // Instantiate once to prevent GC churn (Performance optimization)
      this._outerGlow = new Konva.Rect({
        stroke: "#ff4500", strokeWidth: 4, cornerRadius: 4,
        shadowColor: "#ff4500", shadowBlur: 20, shadowOpacity: 0.8,
      });

      this._innerGlow = new Konva.Rect({
        stroke: "#ffd700", strokeWidth: 2, cornerRadius: 4,
        shadowColor: "#ffd700", shadowBlur: 10, shadowOpacity: 0.9,
      });
      ...
      this._cursorAnim = new Konva.Animation((frame) => {
        ...
        // Slightly wobble the rects around base position
        this._outerGlow.x(this._outerGlow.baseX + Math.random() * 2 - 1);
        this._outerGlow.y(this._outerGlow.baseY + Math.random() * 2 - 1);
      }, this.cursorLayer);
      this._cursorAnim.start();
    }

    // Update positions and sizes on subsequent calls
    this._outerGlow.baseX = ox + x * cs + 2;
    this._outerGlow.baseY = oy + y * cs + 2;
    this._outerGlow.setAttrs({ width: cs - 4, height: cs - 4 });

    this._innerGlow.setAttrs({
      x: ox + x * cs + 6, y: oy + y * cs + 6,
      width: cs - 12, height: cs - 12
    });
  }
