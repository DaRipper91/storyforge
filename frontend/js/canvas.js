...
export class GridCanvas {
  ...
  _renderCursor() {
    if (!this.state) {
      if (this._cursorOuterGlow) {
        this._cursorOuterGlow.hide();
        this._cursorInnerGlow.hide();
      }
      return;
    }
    ...
    // Initialize cursor nodes and animation once to reduce GC pressure
    if (!this._cursorOuterGlow) {
      this._cursorOuterGlow = new Konva.Rect({
        stroke: "#ff4500",
        strokeWidth: 4,
        cornerRadius: 4,
        shadowColor: "#ff4500",
      ...
      this._cursorInnerGlow = new Konva.Rect({
        stroke: "#ffd700",
        strokeWidth: 2,
        cornerRadius: 4,
        shadowColor: "#ffd700",
      ...
      this.cursorLayer.add(this._cursorOuterGlow);
      this.cursorLayer.add(this._cursorInnerGlow);

      this._cursorAnim = new Konva.Animation((frame) => {
        if (!this.state) return;
        const csLocal = this.cellSize;
        const oxLocal = this.offsetX, oyLocal = this.offsetY;
        const xLocal = this.cursor.x, yLocal = this.cursor.y;

        const flicker = Math.sin(frame.time / 50) * 0.2 + 0.8;
        const pulse = Math.sin(frame.time / 100) * 2;

        this._cursorOuterGlow.shadowBlur(20 + pulse * 4);
        this._cursorOuterGlow.opacity(0.6 + flicker * 0.4);

        this._cursorInnerGlow.shadowBlur(10 + pulse * 2);
        this._cursorInnerGlow.opacity(0.7 + flicker * 0.3);

        // Slightly wobble the rects
        this._cursorOuterGlow.x(oxLocal + xLocal * csLocal + 2 + Math.random() * 2 - 1);
        this._cursorOuterGlow.y(oyLocal + yLocal * csLocal + 2 + Math.random() * 2 - 1);
      }, this.cursorLayer);
      this._cursorAnim.start();
    }

    // Ensure they are visible and update properties
    this._cursorOuterGlow.show();
    this._cursorOuterGlow.setAttrs({
      x: ox + x * cs + 2,
      y: oy + y * cs + 2,
      width: cs - 4,
      height: cs - 4,
    ...
    this._cursorInnerGlow.show();
    this._cursorInnerGlow.setAttrs({
      x: ox + x * cs + 6,
      y: oy + y * cs + 6,
      width: cs - 12,
      height: cs - 12,
