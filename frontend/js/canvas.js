...
export class GridCanvas {
  ...
  _renderCursor() {
    ...
    // Initialize nodes and animation once to prevent GC churn
    if (!this._outerGlow) {
      ...
      this._cursorAnim = new Konva.Animation((frame) => {
        ...
        if (this._cursorBaseX !== undefined) {
    ...
    // Update positions and sizes instead of re-creating
    this._cursorBaseX = ox + x * cs + 2;
    this._cursorBaseY = oy + y * cs + 2;

    this._outerGlow.x(this._cursorBaseX);
    this._outerGlow.y(this._cursorBaseY);
