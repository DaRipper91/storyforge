...
export class GridCanvas {
  ...
  _startAmbientParticles() {
    // ⚡ Bolt: Pre-allocate Object Pool to prevent GC churn and skip generic layer iteration
    const POOL_SIZE = 30;
    this._particlePool = Array.from({ length: POOL_SIZE }, () => {
      const p = new Konva.Circle({
        fill: '#ffaa00', shadowColor: '#ff0000', shadowBlur: 5,
        listening: false, visible: false // ⚡ Bolt: Disable hit graph for perf
      });
      this.fxLayer.add(p);
      return p;
    });
    ...
    this._particleAnim = new Konva.Animation((frame) => {
      if (Math.random() < 0.05) {
        const p = this._particlePool.find(node => !node.visible());
        if (p) {
          const sx = this.stage.x(), sy = this.stage.y();
          p.setAttrs({
            x: (Math.random() * this.stage.width()) - sx,
            y: (this.stage.height() + 10) - sy,
      ...
      for (let i = 0; i < POOL_SIZE; i++) {
        const node = this._particlePool[i];
        ...
        if (node.opacity() <= 0 || node.y() < -10) node.visible(false);
      }
    }, this.fxLayer);
