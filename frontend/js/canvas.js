...
export class GridCanvas {
  ...
  _startAmbientParticles() {
    // ⚡ Bolt: Use an isolated object pool to prevent garbage collection churn
    // and avoid iterating over generic fxLayer children.
    ...
    for (let i = 0; i < MAX_PARTICLES; i++) {
      const p = new Konva.Circle({
        ...
        shadowColor: '#ff0000',
        shadowBlur: 5,
        listening: false, // ⚡ Bolt: Remove from hit graph for performance
      });
      p.active = false;
      p.hide();
      pool.push(p);
    ...
    this._particleAnim = new Konva.Animation((frame) => {
      ...
      if (Math.random() < 0.05) {
        ...
        const p = pool.find(part => !part.active);
        if (p) {
