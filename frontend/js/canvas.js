...
export class GridCanvas {
  ...
  _renderCursor() {
    ...
    // Initialize cursor graphics once
    ...
    if (!this._cursorAnim) {
      this._cursorAnim = new Konva.Animation((frame) => {
        ...
        // Slightly wobble the rects (re-calculate base position dynamically)
        const d_cs = this.cellSize;
        const d_ox = this.offsetX;
        const d_oy = this.offsetY;
        const { x: cx, y: cy } = this.cursor;

        this._cursorOuterGlow.x(d_ox + cx * d_cs + 2 + Math.random() * 2 - 1);
        this._cursorOuterGlow.y(d_oy + cy * d_cs + 2 + Math.random() * 2 - 1);
      }, this.cursorLayer);
