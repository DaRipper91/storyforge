  ...
  _renderCursor() {
    ...
    if (!this._cursorAnim) {
      this._cursorAnim = new Konva.Animation((frame) => {
        if (!this._outerGlow || !this._innerGlow) return;
        ...
        // Base X and Y are updated in setAttrs above, we add a tiny wobble to the current base
        const currentBaseOuterX = this.offsetX + this.cursor.x * this.cellSize + 2;
        const currentBaseOuterY = this.offsetY + this.cursor.y * this.cellSize + 2;
        this._outerGlow.x(currentBaseOuterX + Math.random() * 2 - 1);
        this._outerGlow.y(currentBaseOuterY + Math.random() * 2 - 1);
      }, this.cursorLayer);
