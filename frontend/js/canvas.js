...
export class GridCanvas {
  ...
  _renderCursor() {
    ...
    // Initialize the cursor shapes and animation only once to avoid garbage collection churn
    if (!this._cursorOuterGlow) {
      ...
      this._cursorAnim = new Konva.Animation((frame) => {
        ...
        // Slightly wobble the outer rect based on current position
        this._cursorOuterGlow.x(this.offsetX + this.cursor.x * this.cellSize + 2 + Math.random() * 2 - 1);
        this._cursorOuterGlow.y(this.offsetY + this.cursor.y * this.cellSize + 2 + Math.random() * 2 - 1);
      }, this.cursorLayer);

      this._cursorAnim.start();
    }

    // Update the position and size of the cursor shapes instead of recreating them
