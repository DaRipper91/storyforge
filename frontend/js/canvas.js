...
export class GridCanvas {
  ...
  _renderCursor() {
    ...
    const baseX = ox + x * cs + 2;
    const baseY = oy + y * cs + 2;

    if (!this._outerGlow) {
      // Optimization: Cache and reuse Konva nodes to prevent GC churn on frequent cursor updates
      // Outer flame glow
      ...
      this._cursorAnim = new Konva.Animation((frame) => {
        if (!this._outerGlow || !this._outerGlow.isVisible()) return;
        ...
        // Slightly wobble the rects around base
    ...
    // Update geometry (tracking base coordinates explicitly instead of decorating Konva nodes)
    this._cursorBaseX = baseX;
    this._cursorBaseY = baseY;
    this._outerGlow.x(baseX);
    this._outerGlow.y(baseY);
