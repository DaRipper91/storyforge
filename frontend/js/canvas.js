...
export class GridCanvas {
  ...
  _renderCursor() {
    if (!this.state) {
      if (this._outerGlow) this._outerGlow.hide();
      if (this._innerGlow) this._innerGlow.hide();
    ...
    const basexOuter = ox + x * cs + 2;
    const baseyOuter = oy + y * cs + 2;
    const basexInner = ox + x * cs + 6;
    const baseyInner = oy + y * cs + 6;

    if (!this._outerGlow) {
      // Initialize Outer flame glow
      ...
      this._cursorAnim = new Konva.Animation((frame) => {
        ...
        // Slightly wobble the rects. We recalculate base positions here
        // so the wobble is relative to the current cursor position.
        const current_ox = this.offsetX;
        const current_oy = this.offsetY;
        const current_cs = this.cellSize;
        const current_x = this.cursor.x;
        const current_y = this.cursor.y;

        const current_basexOuter = current_ox + current_x * current_cs + 2;
        const current_baseyOuter = current_oy + current_y * current_cs + 2;

        this._outerGlow.x(current_basexOuter + Math.random() * 2 - 1);
        this._outerGlow.y(current_baseyOuter + Math.random() * 2 - 1);
      }, this.cursorLayer);
    }

    // Update dimensions and base positions
    this._outerGlow.show();
    this._outerGlow.x(basexOuter);
    this._outerGlow.y(baseyOuter);
    this._outerGlow.width(cs - 4);
    this._outerGlow.height(cs - 4);

    this._innerGlow.show();
    this._innerGlow.x(basexInner);
    this._innerGlow.y(baseyInner);
    this._innerGlow.width(cs - 12);
    this._innerGlow.height(cs - 12);

    if (!this._cursorAnim.isRunning()) {
