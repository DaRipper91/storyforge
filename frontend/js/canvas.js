...
export class GridCanvas {
  ...
  _renderCursor() {
    ...
    let outerGlow = this.cursorLayer.findOne('#cursor-outer-glow');
    let innerGlow = this.cursorLayer.findOne('#cursor-inner-glow');

    if (!outerGlow) {
      outerGlow = new Konva.Rect({
        id: 'cursor-outer-glow',
        stroke: "#ff4500",
        strokeWidth: 4,
        cornerRadius: 4,
        shadowColor: "#ff4500",
      ...
      this._cursorAnim = new Konva.Animation((frame) => {
        ...
        // Slightly wobble the rects using explicit instance state
        outerGlow.x(this._cursorBaseX + Math.random() * 2 - 1);
        outerGlow.y(this._cursorBaseY + Math.random() * 2 - 1);
      }, this.cursorLayer);
      this._cursorAnim.start();
    }

    // Update base dimensions/positions on move/resize
    outerGlow.x(this._cursorBaseX);
    outerGlow.y(this._cursorBaseY);
    outerGlow.width(cs - 4);
    outerGlow.height(cs - 4);

    innerGlow.x(this._cursorBaseX + 4);
    innerGlow.y(this._cursorBaseY + 4);
